...
pub fn InlineVm(comptime Grammar: type, comptime config: struct { debug: type = dbg.SilentDebug }) type {
    const P = ProgramFor(Grammar);
    const DS = dbg.DebugSupport(P);
    ...
    return struct {
        ...
        // debug helpers were moved to src/debug.zig -> dbg.DebugSupport(P)
                ...
                inline 0...P.code.len - 1 => |k| {
                    ...
                    const formatted_trail: []const u8 = blk: {
                        if (comptime @hasDecl(config.debug, "wantsTrail") and config.debug.wantsTrail) {
                            ...
                            try DS.formatTrail(self.trail.items, trail_fbs.writer());
                    ...
                    if (comptime @hasDecl(config.debug, "onState")) {
                        ...
                        try DS.renderGrammarState(k, gfb.writer());
                        const gline = gfb.getWritten();
                        const rn = DS.ipToRuleName(k);
                        const ro = DS.ipToRuleOffset(k);
...
// Debug printers moved to src/debug.zig; re-export for compatibility
pub const SilentDebug = dbg.SilentDebug;

pub const VerboseDebug = dbg.VerboseDebug;
...
pub const TraceDebug = dbg.TraceDebug;

/// Concise state-by-state grammar-focused debug output (re-export)
pub const StateDebug = dbg.StateDebug;

/// Grammar-specific combinators + Op generator
pub fn Combinators(comptime G: type, comptime config: struct { debug: type = dbg.SilentDebug }) type {
